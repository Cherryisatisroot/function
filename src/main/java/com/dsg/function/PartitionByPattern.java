...
public class PartitionByPattern extends AbstractPartitionAlgorithm implements RuleAlgorithm {
    ...
    //依据配置文件保存对应值
    //[{nodeIndex,valueStart,valueEnd,hashCode},{nodeIndex,valueStart,valueEnd,hashCode}......]
    ...
    /**
     * 根据传入的hash判断是否在longRang的end和start范围中，存在就返回对应的nodeIndex，反之返回null
     * @param hash
     * @return
     */
     ...
     * @param columnValue
     * @return 返回对应的分区值
     * 1.当传入的参数为 null , NULL, 非数字时根据默认节点的设置来返回null 或者 默认节点值
     * 2.根据设置的patternValue和传入的参数做取模运算求hash值，调用findNode方法计算结果
     */
    @Override
    ...
    */
     ...
     * @param endValue  结束值
     * @return
     * 1.判断传入的参数是否为数字类型字符串，不是则返回allNode(所有节点)
     * 2.用设置的patternValue分别和参数做取余运算赋值给hbv和hev
     * 3.调用findNode判断计算后的hbv和hev是否为null，是则返回所有节点（type3）
     * 4.bv和ev做比较运算来确定返回值
     */
     ...
     * 初始化
     * 1.根据配置文件
     *      哈希集合ids中保存配置的data node index
     *      longRanges数组保存[{nodeIndex,valueStart,valueEnd,hashCode},{nodeIndex,valueStart,valueEnd,hashCode},{nodeIndex,valueStart,valueEnd,hashCode},......]
     *      propertiesMap保存配置文件信息，patternValue和defaultNode值
     */
